...
static int PS2DiscBootViaStage2(const char *boot_path, int skip_PS2LOGO, const char *osdgsm_arg)
{
    ...
    if (osdgsm_arg != NULL && *osdgsm_arg != '\0') {
        snprintf(gsm_opt, sizeof(gsm_opt), "-gsm=%s", osdgsm_arg);
        stage2_argv[stage2_argc++] = gsm_opt;
    }

    DPRINTF("%s: launching embedded stage2 loader with %d args\n", __func__, stage2_argc);
...
int PS2DiscBoot(int skip_PS2LOGO)
{
    ...
    if (PS2DiscBootViaStage2(line, skip_PS2LOGO, osdgsm_arg) == 0)
        return 0;
    DPRINTF("%s: stage2 loader handoff failed, using legacy path\n", __func__);
#endif
    if (skip_PS2LOGO) {
        SifExitCmd();
