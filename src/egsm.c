...
#define _TO_KSEG0(x) ((void *)(((uint32_t)(x) & 0x0fffffff) | 0x80000000))
#define _TO_KSEG1(x) ((void *)(((uint32_t)(x) & 0x0fffffff) | 0xA0000000))

// Registers from interrupted program
uint8_t el2_stack[256] __attribute__((aligned(128)));
...
struct video_mode {
  uint32_t mode : 7;
  uint32_t interlace : 1; // 0 = non-interlaced, 1 = interlaced
  uint32_t ffmd : 1;      // 0 = field, 1 = frame
  uint32_t vck : 3;
  uint32_t reserved : 20;

  uint16_t FBHeight; // 240/480/960 = 1/2/4
  uint16_t DPHeight; // 240/480/960 = 1/2/4
...
struct gsm_state {
  ...
  uint32_t VSINTCount : 4;
  uint32_t VSINTPrev : 1;
  uint32_t VCKDiv : 3;  // VCK    devide   (1x, 2x     or 4x)
  uint32_t MAGHMul : 3; // Width  multiply (1x, 2x, 3x or 4x)

  uint32_t FBMul : 3; // Height multiply (1x, 2x     or 4x)
  uint32_t FBDiv : 3; // Height devide   (1x or 2x)

  uint32_t DPMul : 3; // Height multiply (1x, 2x     or 4x)
  uint32_t DPDiv : 3; // Height devide   (1x or 2x)
...
enum MIPS_OP_SPECIAL { MOPS_SLL = 0x00, MOPS_JR = 0x08, MOPS_JALR = 0x09 };
 ...
 *
 */
static uint64_t mod_DISPLAY(struct gsm_state *pstate, uint64_t r) {
  int dx = (r) & 0xfff;
  int dy = (r >> 12) & 0x7ff;
  int magh = (r >> 23) & 0x00f;
  int magv = (r >> 27) & 0x003;
...
static void update_scaling(struct gsm_state *pstate) {
  ...
  if (pstate->flags & EGSM_FLAG_VMODE_FP1) {
    // 240p/288p mode
    pstate->MAGHMul = 1; // 640 * 1 = 640
  } else if (pstate->flags & EGSM_FLAG_VMODE_FP2) {
    // 480p/576p mode
    pstate->MAGHMul = 1; // 640 * 1 = 640
  } else if (pstate->flags & EGSM_FLAG_VMODE_1080I_X1) {
    // 1080i mode
    pstate->MAGHMul = 1; // 640 * 1 = 640
    pstate->gsm.FBHeight /= 2;
    pstate->gsm.DPHeight /= 2;
  } else if (pstate->flags & EGSM_FLAG_VMODE_1080I_X2) {
    // 1080i mode
    pstate->MAGHMul = 2; // 640 * 2 = 1280
  } else if (pstate->flags & EGSM_FLAG_VMODE_1080I_X3) {
    // 1080i mode
...
void el2_c_handler(ee_registers_t *regs) {
  ...
  if (cop0_Cause & EE_CAUSE_BD2)
    instr = *(uint32_t *)(cop0_ErrorEPC + 4); // Branch delay slot
  else
    instr = *(uint32_t *)(cop0_ErrorEPC);

  // Decode the instruction
  uint32_t op = (instr >> 26) & 0x003f;
  uint32_t rs = (instr >> 21) & 0x001f;
  uint32_t rt = (instr >> 16) & 0x001f;
  uint32_t imm = (instr) & 0xffff;
  uint64_t *dest = NULL;
  uint64_t *source = NULL;
  uint64_t value = 0;
  if (op == MOP_SD) {
    value = (uint64_t)regs->gpr[rt];
    dest = (uint64_t *)((uint32_t)regs->gpr[rs] + imm);
  } else if (op == MOP_LD) {
    source = (uint64_t *)((uint32_t)regs->gpr[rs] + imm);
  } else {
    _ee_disable_bpc();
    return;
  }
  ...
  uint32_t bpc_save = _ee_disable_bpc();

  // Process LD instruction (read from register)
  switch ((uint32_t)source & 0x1fffffff) {
  case (uint32_t)GS_REG_CSR:
    // Emulate field flipping
    uint64_t csr = *source;
    uint32_t VSINT = (csr >> 3) & 1;
    uint32_t FIELD_emu = (csr >> 13) & 1;

    if (pstate->flags & EGSM_FLAG_COMP_1) {
      //
    ...
    } else if (pstate->flags & EGSM_FLAG_COMP_2) {
      //
      ...
      FIELD_emu = pstate->VSINTCount & 1;
    } else if (pstate->flags & EGSM_FLAG_COMP_3) {
      //
      // mode 3: invert FIELD
      //
      // FIELD_emu = !FIELD_emu;
  ...
  case 0:
    break;
  case (uint32_t)GS_REG_PMODE:
  case (uint32_t)GS_REG_SMODE2:
  case (uint32_t)GS_REG_DISPLAY1:
  case (uint32_t)GS_REG_DISPLAY2:
  case (uint32_t)GS_REG_SIGLBLID:
    regs->gpr[rt] = *source;
    break;
  default:
  }
  ...
  switch ((uint32_t)dest & 0x1fffffff) {
  case (uint32_t)GS_REG_DISPLAY1:
  ...
  if (cop0_Cause & EE_CAUSE_BD2) {
    ...
    uint32_t branch_instr = *(uint32_t *)(cop0_ErrorEPC);
    // Decode the instruction
    op = (branch_instr >> 26) & 0x003f;     // OP
    rs = (branch_instr >> 21) & 0x001f;     // Register source
    rt = (branch_instr >> 16) & 0x001f;     // Register temp
    uint32_t ops = (branch_instr) & 0x003f; // Special OP
    int32_t offset = (branch_instr & 0xffff);
    if (offset & 0x8000) {
      // Negative
      offset = ~((offset ^ 0xffff) + 1) + 1;
    }
    uint32_t instr_index = (branch_instr << 2) & 0x0fffffff;
...
static void hook_SetGsCrt(short int interlace, short int mode, short int ffmd) {
  ...
  if (pstate->flags & EGSM_FLAG_VMODE_FP1) {
    ...
    if (pstate->game.mode == 3 && (pstate->flags & EGSM_FLAG_NO_576P))
      return;
  ...
  } else if (pstate->flags & (EGSM_FLAG_VMODE_1080I_X1 | EGSM_FLAG_VMODE_1080I_X2 | EGSM_FLAG_VMODE_1080I_X3)) {
  ...
  // Enable breakpoints
  if (pstate->flags & (EGSM_FLAG_COMP_1 | EGSM_FLAG_COMP_2 | EGSM_FLAG_COMP_3)) {
